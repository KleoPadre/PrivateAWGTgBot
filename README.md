...
- ✅ Проверит наличие Docker и контейнера AmneziaWG
- ✅ Извлечет все параметры из конфигурации сервера
- ✅ Определит внешний IP адрес сервера
- ✅ Создаст `.env` файл со всеми настройками

Вам нужно будет указать только:
...
2. **ADMIN_ID** - ваш Telegram ID:
   ...
   - Отправьте `/start` и получите ваш ID
   
3. **USERS** - список разрешенных пользователей (опционально)
...
⚠️ **Внимание**: Кнопка "🔄 Перезагрузить сервер" выполняет полную перезагрузку Ubuntu. Бот автоматически запустится после перезагрузки (автозапуск включен).
